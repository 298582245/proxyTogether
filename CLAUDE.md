...
| 主AI调用Codex MCP执行审查,传递完整审查清单与验收标准 | 审查执行沿用 `mcp\_\_codex\_\_codex` 调用格式 |
...
| 固定使用 `mcp\_\_codex\_\_codex model="gpt-5-codex" sandbox="danger-full-access" prompt="<TASK>"` 发起结构化调用，不得改动参数顺序。 | 与 AGENTS.md 保持一致 |

| 结果写入 `.claude/structured-request.json`，填充结构化需求模板字段：meta.recorded\_at、meta.recorder、meta.source、objective、scope.in\_scope、scope.out\_of\_scope、deliverables、dependencies、risks、questions。 | 与 AGENTS.md、standard-workflow 保持一致 |
...
\- ✅ 调用 Codex 进行深度分析（mcp\_\_codex\_\_codex 或 codex-reply）
...
\- 优先：Codex MCP Server（如果已配置 `mcp\_\_codex\_\_codex`）

\- 降级：Bash CLI（`codex --prompt=...` 作为稳定 fallback）

\- conversationId 管理：使用 task\_marker 机制或持久化到 `.claude/codex-sessions.json`
...
&#x20;   "current\_task\_id": "task-123",

&#x20;   "files\_modified": \["src/foo.ts"],

&#x20;   "last\_update": "2025-10-03T10:30:00Z",

&#x20;   "status": "coding|review\_needed|completed",

&#x20;   "pending\_questions": \["如何处理边界情况X？"],

&#x20;   "complexity\_estimate": "simple|moderate|complex"
...
3\. 主AI 完成编码 → 标记 `review\_needed` → 触发 Codex 审查
...
\- \*\*主AI\*\*：生成task\_marker、调用Codex、记录conversationId
...
1\. 生成唯一task\_marker：`\[TASK\_MARKER: YYYYMMDD-HHMMSS-XXXX]`

2\. 将task\_marker嵌入prompt开头（第一行）

3\. 调用mcp\_\_codex\_\_codex

4\. 从响应提取`\[CONVERSATION\_ID]: xxx`
...
1\. 提取task\_marker（prompt第一行）

2\. 查询conversationId：遍历`\~/.codex/sessions`目录下最近5个会话文件，匹配task\_marker

3\. 持久化到`.claude/codex-sessions.json`

4\. 在响应末尾附加：`\[CONVERSATION\_ID]: <conversationId>`

5\. 如果未找到：`\[CONVERSATION\_ID]: NOT\_FOUND`
...
\- 调用`mcp\_\_codex\_\_codex-reply(conversationId=xxx, prompt=yyy)`
...
\- NOT\_FOUND：记录到operations-log.md，创建新会话
...
\- ✅ 调用 mcp\_\_codex\_\_codex 或 codex-reply
